...
This document describes the technical architecture of the Napco iBridge custom component for Home Assistant.

## Directory Structure

```text
custom_components/napco_ibridge/
...
│   ├── __init__.py          # Exports NapcoIbridgeDataUpdateCoordinator
...
│   ├── __init__.py          # Exports NapcoIbridgeEntity
...
- `base.py` - Main coordinator class (`NapcoIbridgeDataUpdateCoordinator`)
...
**Key class:** `NapcoIbridgeDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
**Key class:** `NapcoIbridgeApiClient`
...
- `NapcoIbridgeConfigFlowHandler` (main flow)
- `NapcoIbridgeOptionsFlow` (options)
...
**Key class:** `NapcoIbridgeEntity` (in `entity/base.py`)
...
2. `NapcoIbridgeEntity` for common functionality
...
1. Create directory: `custom_components/napco_ibridge/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `NapcoIbridgeEntity`
