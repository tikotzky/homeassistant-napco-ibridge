...
This document describes all configuration options and settings available in the Napco iBridge custom integration.
...
```yaml
service: napco_ibridge.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: napco_ibridge.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
- [GitHub Issues](https://github.com/mordytk/homeassistant-napco-ibridge/issues) - Report problems
