...
**Directory:** `custom_components/napco_ibridge/[platform]/`
...
```python
"""[Platform] platform for Napco iBridge."""
...
from .[entity_file] import NapcoIbridge[EntityName]
...
async def async_setup_entry(
    ...
    coordinator: NapcoIbridgeDataUpdateCoordinator = hass.data[DOMAIN][
        entry.entry_id
    ]

    async_add_entities(
        [
            NapcoIbridge[EntityName](coordinator, entry),
...
```python
"""[Entity description] for Napco iBridge."""
...
Add platform to `custom_components/napco_ibridge/manifest.json`:
...
- [ ] Entity class inherits from both `NapcoIbridgeEntity` and platform class
...
- **Base entity:** `NapcoIbridgeEntity` in `entity/base.py`
- **Coordinator:** `NapcoIbridgeDataUpdateCoordinator`
