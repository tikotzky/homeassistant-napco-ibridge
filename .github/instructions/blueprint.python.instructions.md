...
- Files: `snake_case.py`
- Classes: `PascalCase` prefixed with `NapcoIbridge`
...
- Inherit from both platform entity and `NapcoIbridgeEntity` (order matters)
...
from custom_components.napco_ibridge.sensor import async_setup_entry
...
**Config entry data:** `entry_data: NapcoIbridgeData = hass.data[DOMAIN][entry.entry_id]`
