...
- **Title:** Napco iBridge
- **Repository:** mordytk/homeassistant-napco-ibridge

**Key directories:**

- `custom_components/napco_ibridge/` - Main integration code
...
- Integration logs: `custom_components.napco_ibridge: debug` in `config/configuration.yaml`
...
- Use the domain `napco_ibridge` for all DOMAIN references
- Prefix all integration-specific classes with `NapcoIbridge`
- Use "Napco iBridge" as the display title
...
- Inherit from platform base + `NapcoIbridgeEntity`
...
- `tests/` mirrors `custom_components/napco_ibridge/` structure
